...
from foveate.cascade import _cls_survivors, _mask_overlap, _nms, _split_kmeans, _survivors
...
def test_cls_survivors_stop_rule():
    # No child beats the parent → empty → the parent is the CLS peak (cls-stop fires).
    assert _cls_survivors(0.8, [0.7, 0.8, 0.75]) == []      # ties (0.8) do NOT survive
    # Only children strictly above the parent continue; the lower-sim sibling is dropped.
    assert _cls_survivors(0.6, [0.7, 0.5, 0.9]) == [0, 2]
    # A single worse child (the plain zoom case) → stop and emit the predecessor.
    assert _cls_survivors(0.75, [0.6]) == []
    # A single better child → keep zooming, parent superseded.
    assert _cls_survivors(0.5, [0.55]) == [0]
...
def test_survivors_single_child_is_strict_zoom_guard():
    ...
    assert _survivors(0.8, [0.7], cls_threshold=0.5) == []      # over-zoomed → stop
    assert _survivors(0.5, [0.55], cls_threshold=0.5) == [0]    # improved → keep zooming
...
def test_survivors_keeps_novel_sibling_below_biased_parent():
    ...
    keep = _survivors(0.85, [0.90, 0.62], cls_threshold=0.5)
...
def test_survivors_rejects_fragmented_single_object():
    ...
    assert _survivors(0.90, [0.80, 0.75], cls_threshold=0.5) == []    # partial halves, both worse
    assert _survivors(0.90, [0.90, 0.90], cls_threshold=0.5) == []    # flat/tied CLS never confirms


def test_cls_worse_children_are_traced(backbone, two_squares, monkeypatch):
    """Dropped (CLS-worse) children must be emitted as terminal ``cls-worse`` events so the
    trajectory / tool can show why the cascade stopped."""
    import foveate.cascade as C

    # Force "no child ever survives" → the cls-stop path fires and every child is dropped.
    monkeypatch.setattr(C, "_survivors",
                        lambda parent_cls, child_scores, *, cls_threshold: [])
    img, ex = two_squares
    events = []
    C.discover_instances(backbone, img, ex,
                         config=Config(min_crop=24, cascade_min_instance_area=4),
                         observer=events.append)
    worse = [e for e in events if e["decision"] == "cls-worse"]
    assert worse, "expected cls-worse events for dropped children"
    for e in worse:                                          # well-formed terminal trace nodes
        assert "parent_cls" in e and e["children"] == [] and e["instance_grids"] == []
        assert "box" in e and "cls_score" in e
    assert any(e["decision"] == "cls-stop" for e in events)  # the predecessor was emitted instead
...
def test_nms_disabled_when_thresholds_are_one(backbone, two_squares):
    ...
    img, ex = two_squares
    _, stats = discover_instances(backbone, img, ex,
                                  config=Config(min_crop=24, cascade_min_instance_area=4,
...
def test_zoom_split_retry_attempts_split_on_marginal_peak(backbone, two_squares):
    ...
    eps the retry fires (a ``clump-split`` appears where a plain ``cls-stop`` would be), yet the
    ...
    ev0 = []; inst0, st0 = discover_instances(backbone, img, ex, config=off, observer=ev0.append)
    on = Config(min_crop=24, cascade_min_instance_area=4, zoom_split_retry_eps=1.0)
    ev1 = []; inst1, st1 = discover_instances(backbone, img, ex, config=on, observer=ev1.append)
...
def test_max_depth_is_not_a_stopping_signal(backbone, two_squares):
    ...
    img, ex = two_squares
    events = []
    discover_instances(backbone, img, ex,
