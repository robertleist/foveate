...
def test_cls_bank_shape_and_norm(backbone, two_squares, name):
    img, ex = two_squares
    cfg = Config(foreground_extractor=name, standardize=True)
    extr = build_extractor(cfg)
    extr.set_reference(backbone, img, ex, None, cfg)

    cls_bank = extr.cls_bank
    assert cls_bank.ndim == 2                              # (S, D)
    assert cls_bank.shape[0] == len(ex)                    # one row per exemplar mask
    norms = cls_bank.norm(dim=1)
...
def test_leaf_classification_is_mean_cosine():
    ...
    torch.manual_seed(0)
    s, d = 4, 8
    cls_bank = l2_normalize(torch.randn(s, d), dim=1)      # (S, D) unit rows
    target = l2_normalize(torch.randn(d), dim=0)           # (D,)

    explicit = sum(float(target @ cls_bank[i]) for i in range(s)) / s
    matrix = float((target @ cls_bank.T).mean())
